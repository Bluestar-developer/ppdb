    ...
    <div class="relative z-10 max-w-7xl mx-auto py-8 px-4 sm:px-6 lg:px-8 space-y-6">
        ...
        <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 reveal">
            <div>
                ...
                <h1 class="text-2xl md:text-3xl font-black text-gray-900 tracking-tight">Jadwal PPDB</h1>
                <p class="text-gray-500 text-sm mt-1">Kelola agenda kegiatan Penerimaan Peserta Didik Baru</p>
            </div>
            <a href="{{ route('admin.jadwal.create') }}" class="btn-premium inline-flex items-center gap-2">
            ...
            <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-5 hover:shadow-md transition-all duration-300">
                <div class="flex items-center gap-3">
                    ...
                    <div>
                        ...
                        <p class="text-xl font-bold text-gray-900">{{ $jadwal->count() ?? 0 }}</p>
                    </div>
                </div>
            </div>
            <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-5 hover:shadow-md transition-all duration-300">
                <div class="flex items-center gap-3">
                    ...
                    <div>
                        ...
                        <p class="text-xl font-bold text-gray-900">{{ $jadwal->filter(function($j) { return \Carbon\Carbon::parse($j->tanggal_selesai)->gte(now()); })->count() ?? 0 }}</p>
                    </div>
                </div>
            </div>
            <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-5 hover:shadow-md transition-all duration-300">
                <div class="flex items-center gap-3">
                    ...
                    <div>
                        ...
                        <p class="text-xl font-bold text-gray-900">{{ $jadwal->filter(function($j) { return \Carbon\Carbon::parse($j->tanggal_selesai)->lt(now()); })->count() ?? 0 }}</p>
                    </div>
                </div>
            </div>
        </div>
        ...
        <div class="bg-white rounded-3xl shadow-sm border border-gray-100 overflow-hidden hover:shadow-lg transition-all duration-500 reveal">
                ...
                <table class="w-full">
                    <thead>
                        <tr class="bg-gray-50/80 border-b border-gray-100">
                            <th class="px-6 py-3.5 text-left text-[11px] font-bold text-gray-500 uppercase tracking-wider">Nama Kegiatan</th>
                            <th class="px-6 py-3.5 text-left text-[11px] font-bold text-gray-500 uppercase tracking-wider">Tanggal Mulai</th>
                            <th class="px-6 py-3.5 text-left text-[11px] font-bold text-gray-500 uppercase tracking-wider">Tanggal Selesai</th>
                            <th class="px-6 py-3.5 text-left text-[11px] font-bold text-gray-500 uppercase tracking-wider">Keterangan</th>
                    ...
                    <tbody class="divide-y divide-gray-50">
                        @forelse($jadwal as $j)
                        @php
                            $tglMulai = \Carbon\Carbon::parse($j->tanggal_mulai);
                            $tglSelesai = \Carbon\Carbon::parse($j->tanggal_selesai);
                            $now = now();
                            $isActive = $tglSelesai->gte($now);
                            $isOngoing = $tglMulai->lte($now) && $tglSelesai->gte($now);
                        @endphp
                        <tr class="group hover:bg-blue-50/30 transition-all duration-200">
                            <td class="px-6 py-3.5">
                                <div class="flex items-center gap-2.5">
                                    <div class="w-8 h-8 rounded-full bg-gradient-to-br from-blue-100 to-indigo-100 flex items-center justify-center text-blue-700 font-bold text-xs">
                                        {{ strtoupper(substr($j->nama_kegiatan, 0, 1)) }}
                                    </div>
                                    <div>
                                        <span class="text-sm font-semibold text-gray-900">{{ $j->nama_kegiatan }}</span>
                                        @if($isOngoing)
                                            <span class="ml-2 inline-flex items-center px-1.5 py-0.5 rounded text-[10px] font-bold bg-emerald-100 text-emerald-700">Berlangsung</span>
                                        @endif
                                    </div>
                                </div>
                            </td>
                            <td class="px-6 py-3.5">
                                <div class="flex items-center gap-2">
                                    ...
                                    <span class="text-xs font-medium text-gray-600">{{ $tglMulai->format('d M Y') }}</span>
                                </div>
                            </td>
                            <td class="px-6 py-3.5">
                                <div class="flex items-center gap-2">
                                    ...
                                    <span class="text-xs font-medium text-gray-600">{{ $tglSelesai->format('d M Y') }}</span>
                                </div>
                            </td>
                            <td class="px-6 py-3.5">
                                <span class="text-xs text-gray-500 max-w-xs truncate block" title="{{ $j->keterangan }}">
                                    {{ $j->keterangan ?? '-' }}
                                </span>
                            </td>
                            <td class="px-6 py-3.5">
                            ...
                            <td class="px-6 py-3.5">
                                <div class="flex items-center justify-center gap-2">
                                    ...
                                    <form action="{{ route('admin.jadwal.destroy', $j) }}" method="POST" class="inline" id="deleteForm-{{ $j->id }}">
                                        @csrf @method('DELETE')
                                        <button type="button"
                                                onclick="openDeleteModal(document.getElementById('deleteForm-{{ $j->id }}'), '{{ $j->nama_kegiatan }}')"
                        ...
                        <tr>
                            <td colspan="6" class="px-6 py-16 text-center">
                                ...
                                <p class="text-gray-500 font-medium text-sm">Belum ada jadwal PPDB</p>
                                <p class="text-gray-400 text-xs mt-1">Klik "Tambah Jadwal" untuk memulai</p>
                            </td>
                        </tr>
                        @endforelse
                    </tbody>
            ...
            <div class="px-6 py-4 border-t border-gray-50 bg-gray-50/30 flex flex-col sm:flex-row justify-between items-center gap-4">
                <span class="text-xs text-gray-500">Menampilkan {{ $jadwal->firstItem() ?? 0 }} - {{ $jadwal->lastItem() ?? 0 }} dari {{ $jadwal->total() ?? 0 }} data</span>
                <div class="flex items-center gap-2">
        ...
        <div class="flex min-h-full items-center justify-center p-4 text-center sm:p-0">
            <div class="relative transform overflow-hidden rounded-3xl bg-white text-left shadow-2xl transition-all sm:my-8 sm:w-full sm:max-w-md opacity-0 scale-95" id="modalPanel">
                <div class="bg-rose-50 px-6 py-8 flex flex-col items-center">
                    ...
                    <h3 class="text-lg font-bold text-gray-900" id="modal-title">Hapus Jadwal</h3>
                </div>
                <div class="px-6 py-6">
                    <p class="text-sm text-gray-600 text-center leading-relaxed">
                        Apakah Anda yakin ingin menghapus jadwal <span id="deleteItemName" class="font-semibold text-gray-900"></span>? 
                    ...
                    <div class="mt-4 p-3 rounded-xl bg-amber-50 border border-amber-100 flex items-start gap-3">
                        ...
                        <p class="text-xs text-amber-700 font-medium">Jadwal yang dihapus tidak akan muncul di timeline PPDB.</p>
                    </div>
                </div>
