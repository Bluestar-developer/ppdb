...
<div class="min-h-screen bg-[#f8fafc] relative overflow-hidden">
    ...
    <div class="relative z-10 max-w-7xl mx-auto py-8 px-4 sm:px-6 lg:px-8 space-y-6">
        ...
        <div class="reveal reveal-delay-1">
            ...
            <h1 class="text-2xl md:text-3xl font-black text-gray-900 tracking-tight">Edit Pengumuman</h1>
            <p class="text-gray-500 text-sm mt-1">Perbarui informasi pengumuman: {{ $pengumuman->judul }}</p>
        </div>
        ...
        <div class="bg-white rounded-3xl shadow-sm border border-gray-100 overflow-hidden reveal reveal-delay-2">
            ...
            <form action="{{ route('admin.pengumuman.update', $pengumuman) }}" method="POST" enctype="multipart/form-data" class="p-6 md:p-8 lg:p-10">
                @csrf
                @method('PUT')
                    ...
                    <div class="lg:col-span-2 space-y-6">
                        ...
                        <div>
                            ...
                            <div class="relative">
                                ...
                                <input type="text" name="judul" value="{{ $pengumuman->judul }}" required placeholder="Masukkan judul pengumuman..."
                        ...
                        <div class="grid grid-cols-1 sm:grid-cols-2 gap-6">
                            <div>
                                ...
                                <div class="relative">
                                    ...
                                    <input type="date" name="tanggal_mulai" value="{{ $pengumuman->tanggal_mulai ? $pengumuman->tanggal_mulai->format('Y-m-d') : date('Y-m-d') }}" required
                            ...
                            <div>
                                ...
                                <div class="relative">
                                    ...
                                    <input type="date" name="tanggal_selesai" value="{{ $pengumuman->tanggal_selesai ? $pengumuman->tanggal_selesai->format('Y-m-d') : date('Y-m-d') }}" required
                                           class="w-full pl-10 pr-4 py-2.5 bg-gray-50/80 border border-gray-200 rounded-xl text-sm focus:bg-white focus:border-blue-500 focus:ring-4 focus:ring-blue-500/10 transition-all outline-none">
                                </div>
                            </div>
                        </div>
                        ...
                        <div>
                            ...
                            <div class="relative">
                                ...
                                <textarea name="isi" rows="8" required placeholder="Tulis detail pengumuman di sini..."
                                          class="w-full pl-10 pr-4 py-2.5 bg-gray-50/80 border border-gray-200 rounded-xl text-sm focus:bg-white focus:border-blue-500 focus:ring-4 focus:ring-blue-500/10 transition-all outline-none resize-none placeholder:text-gray-400">{{ $pengumuman->isi }}</textarea>
                            </div>
                        ...
                        <div class="flex items-center gap-3 p-4 rounded-xl bg-gray-50/80 border border-gray-100 max-w-md">
                            <div class="relative inline-flex items-center cursor-pointer">
                                <input type="checkbox" name="is_published" id="is_published" value="1" {{ $pengumuman->is_published ? 'checked' : '' }} class="sr-only peer">
                            ...
                            <div>
                                <label for="is_published" class="text-sm font-semibold text-gray-700 cursor-pointer">Publikasikan</label>
                                <p class="text-xs text-gray-500">Pengumuman akan tampil di halaman publik</p>
                            </div>
                        </div>
                    </div>
                        ...
                        <div class="space-y-4">
                            ...
                            <div id="current-image" class="relative">
                                <div class="aspect-[16/9] rounded-2xl overflow-hidden border border-gray-200 bg-gray-100">
                                    <img src="{{ Storage::url($pengumuman->gambar) }}" class="w-full h-full object-cover" alt="Gambar Saat Ini">
                                </div>
                                <div class="absolute bottom-0 left-0 right-0 bg-gradient-to-t from-black/60 to-transparent p-4">
                                    <p class="text-white text-xs font-medium">Gambar Saat Ini</p>
                                </div>
                                <div class="absolute top-3 right-3">
                                    <span class="px-2 py-1 bg-white/90 backdrop-blur-sm rounded-lg text-xs font-medium text-gray-700 shadow-sm">Aktif</span>
                                </div>
                            </div>
                            ...
                            <!-- Upload Area -->
                            <div class="relative border-2 border-dashed border-gray-200 rounded-2xl p-6 text-center hover:border-blue-300 hover:bg-blue-50/30 transition-all cursor-pointer group" onclick="document.getElementById('gambar').click()">
                                <div class="w-12 h-12 rounded-xl bg-gray-100 group-hover:bg-blue-100 flex items-center justify-center mx-auto mb-3 transition-colors">
                                ...
                                <p class="text-sm text-gray-700 font-semibold">Klik untuk unggah gambar baru</p>
                                <p class="text-xs text-gray-400 mt-1">atau drag & drop file di sini</p>
                                <p class="text-xs text-gray-400 mt-1.5">JPG, PNG, WEBP • Maks 2MB</p>
                            </div>
                            ...
                            <div id="preview-container" class="hidden">
                                <div class="relative">
                                    <div class="aspect-[16/9] rounded-2xl overflow-hidden border border-gray-200 bg-gray-100">
                                        <img id="preview" class="w-full h-full object-cover" src="" alt="Preview Baru">
                                    </div>
                                    <div class="absolute bottom-0 left-0 right-0 bg-gradient-to-t from-black/60 to-transparent p-4">
                                        <p class="text-white text-xs font-medium">Preview Gambar Baru</p>
                                    </div>
...
<script>
    ...
    function clearImage() {
        ...
        // Show current image back if exists
        const currentImg = document.getElementById('current-image');
